...
class GaussianBitDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.proj_vlm = nn.Linear(2048, 512, bias=False) #3584
    ...
    def model_predictions(self, x, pred_x_start_prev, goal_x, goal_pred_x_start_prev, t, obs, stage_masks, gt_goal=None, gt_goal_one_hot=None, index=0, video_file=None,):
        ...
        # ################### npy save ###########################################
        # if t[0] == 0:
        #     save_dir = "outputs/darai_l2/infer_goal"
        ...
        #     txt_path = os.path.join("outputs/darai_l2/phrases_1110", f"phrases_{goal_tag}_{ts}.txt")
        ...
        # ################## npy save ###########################################
        # if t[0] == 0:
        #     save_dir = "outputs/darai_l2/proposed_infer_action"
        #     os.makedirs(save_dir, exist_ok=True)
        #     ts = datetime.now().strftime("%Y%m%d_%H%M%S")
