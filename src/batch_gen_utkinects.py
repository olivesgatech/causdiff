...
class BatchGeneratorTCN(Dataset):
    def __init__(
        ...
        for vid in list_of_vid:
            if obs_perc != 0:
                ...
                self.list_of_examples.append([vid, 0.25])
                self.list_of_examples.append([vid, 0.3])
                self.list_of_examples.append([vid, 0.35])
                self.list_of_examples.append([vid, 0.4])
                self.list_of_examples.append([vid, 0.45])
    ...
    def goal_label_to_id(self, content):
        classes = self.goals_dict[content]
    ...
    def __getitem__(self, idx):
        ...
        goals_label = (os.path.basename(gt_name)).split('_')[-1].split('.')[0]
        ...
        vid_len = len(content)
        seq_paths = [line.split(',')[0] for line in content]
