...
class QwenVLOnlineEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.processor = AutoProcessor.from_pretrained(
            model_id, trust_remote_code=True, cache_dir=cache_dir,  min_pixels=224*224, max_pixels=576*576,
        )
    ...
    def forward(
        ...
        for b in range(B):
            ...
            for t in range(T_b):
                ...
                flat_prompts.append(
                    f"Global goal: {gi_list[b]}. Frame {t}/{total_list[b]}. "
                    "Output <=8 words. Do not repeat the Global goal."
                )
        ...
        with ctx:
            for i0 in range(0, len(flat_imgs), chunk_size):
                ...
                for j in range(i0, i1):
                    ...
                    msgs = [
                        {"role": "system", "content": [{"type": "text", "text": "Output fine-grained intention based only on evidence from two consecutive images. 'Fine-grained' indicates next 1-5s sub-step"}]},
                        {"role": "user", "content": [
    ...
    def generate_phrases(
        ...
        for b in range(B):
            ...
            for t in range(T_b):
                ...
                msgs = [
                    {"role": "system", "content": [
                        {"type": "text", "text": "Output a fine-grained intention (<=8 words) based only on visible evidence."}
                    ]},
                    {"role": "user", "content": [
                        {"type": "image", "image": "<image-placeholder>"},
                        {"type": "text", "text":
