...
from trainers_causal_hierarchical_onlylora_7b_nturgbd import TrainerTCN
...
parser.add_argument('--mapping_coarse_file')
parser.add_argument('--num_highlevel_classes')
...
if args.ds != 'assembly': 
    ...
    # Goals
    mapping_coarse_file = args.mapping_coarse_file
    file_coarse_ptr = open(mapping_coarse_file, 'r')
    actions_coarse = file_coarse_ptr.read().split('\n')[:-1]
    file_coarse_ptr.close()
    goals_dict = dict()
    for a in actions_coarse:
        goals_dict[a.split()[1]] = int(a.split()[0])
    # goals class num
    num_classes_coarse = len(goals_dict)
