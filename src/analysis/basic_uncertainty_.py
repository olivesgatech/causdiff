...
def expected_entropy_over_S(prob_stc: np.ndarray, log_base: float = np.e, eps: float = 1e-12) -> np.ndarray:
    ...
    # (S,T,C) -> (S,T)
    H_st = -np.sum(p * (np.log(p) / denom), axis=-1)
    # S축 평균 -> (T,)
    return H_st.mean(axis=0)
...
def process_pair(b_path: str,
    ...
    # 필요시 확률화
    # bprob = ensure_prob(b, force_softmax=force_softmax)
    # pprob = ensure_prob(p, force_softmax=force_softmax)
    # 입력이 이미 확률이면 아래처럼 바로 사용해도 됨
    bprob = b
    pprob = p

    # Expected Entropy 계산
    b_H_t = expected_entropy_over_S(bprob, log_base=log_base)
    p_H_t = expected_entropy_over_S(pprob, log_base=log_base)
    diff_t = p_H_t - b_H_t
    ...
    # 저장
    np.save(os.path.join(pair_dir, f"baseline_expected_entropy_{unit}.npy"), b_H_t)
    np.save(os.path.join(pair_dir, f"proposed_expected_entropy_{unit}.npy"), p_H_t)
    np.save(os.path.join(pair_dir, f"delta_expected_entropy_{unit}.npy"), diff_t)

    np.savetxt(os.path.join(pair_dir, f"baseline_expected_entropy_{unit}.csv"), b_H_t, delimiter=",", fmt="%.8f")
    np.savetxt(os.path.join(pair_dir, f"proposed_expected_entropy_{unit}.csv"), p_H_t, delimiter=",", fmt="%.8f")
    np.savetxt(os.path.join(pair_dir, f"delta_expected_entropy_{unit}.csv"), diff_t, delimiter=",", fmt="%.8f")

    # 플롯
    plot_two_curves(b_H_t, p_H_t,
                    title=f"Expected Entropy over Time ({unit})",
                    out_png=os.path.join(pair_dir, f"expected_entropy_compare_{unit}.png"),
                    ylabel=f"Expected Entropy ({unit})")
