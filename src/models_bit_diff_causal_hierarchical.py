...
class SinusoidalPositionalEncoding(nn.Module):
    def __init__(self, dim, max_len=4096):
        ...
        self.register_buffer('pe', pe)  # 학습X, 디바이스 따라 이동
...
class BitDiffPredictorTCN(nn.Module):
    ...
    def forward(self, x, t, stage_masks, obs_cond=None, self_cond=None, index=0):
        # arange
        ...
        #### For inference
        frame_wise_pred, frame_wise_feature = self.ms_tcn(x, t, stage_masks, high_level_goal, index=index)#.squeeze(0))
...
class DiffMultiStageModel(nn.Module):
    ...
    def forward(self, x, t, stage_masks, high_level_goal=None, index=0):
        ...
        for sn, s in enumerate(self.stages):
            if self.use_features:
                # 4/14 This is true
                out, out_features = s(torch.cat((F.softmax(out, dim=1) * stage_masks[sn], x), dim=1), t, stage_masks[sn], high_level_goal, index=index)
            else:
...
class DiffSingleStageModel(nn.Module):
    ...
    # 4/14 added obs_cond
    def forward(self, x, t, mask, high_level_goal=None, index=0):
        # embed 
        ...
        for layer in self.layers:
            out = layer(out, time, mask, high_level_goal, index=index)
...
class DiffDilatedGatedResidualLayer(nn.Module):
    ...
    def forward(self, x, t, mask, high_level_goal=None, index=0):
        ...
        if high_level_goal is not None:
            out = self.goal_attn(out, high_level_goal, index=index)  # (B, C, T)
        out = self.conv_1x1(out)
        out = F.relu(out)
        out = self.dropout(out)
...
class CausalGoalAttention(nn.Module):
    def __init__(self, dim, num_heads=4, fusion="sum"):
        ...
        #self.pos_enc = SinusoidalPositionalEncoding(dim, max_len=4096)

        if fusion == "sum":
            # learnable scalar parameter for blending
            self.alpha = nn.Parameter(torch.tensor(0.5))  # starts neutral

    def forward(self, x, goal_embed, index=0):
        """
        x: (B, C, T)
        goal_embed: (B, C)
        """
        ...
        x_hat_raw, cs_weights = self.cs_attn(goal_embed.unsqueeze(1), x_seq, x_seq)  # (B, 1, C)
        x_hat = x_hat_raw.expand(-1, T, -1)
        #save_matrix_npy(np.asarray(cs_weights.detach().cpu()), index=index)
