...
NTURGBD_GOAL = "the intention is to do the daily action"
...
class GaussianBitDiffusion(nn.Module):
    ...
    def p_losses(self,
        ...
        global_goal_texts = NTURGBD_GOAL
    ...
    def model_predictions(self, x, pred_x_start_prev, goal_x, goal_pred_x_start_prev, t, obs, stage_masks, gt_goal=None, gt_goal_one_hot=None, index=0, video_file=None,):
        ...
        # ################### npy save ###########################################
        # if t[0] == 0:
        #     save_dir = "outputs/nturgbd/infer_goal"
        ...
        #     txt_path = os.path.join("outputs/nturgbd/phrases", f"phrases_{goal_tag}_{ts}.txt")
        ...
        # ################## npy save ###########################################
        # if t[0] == 0:
        #     save_dir = "outputs/nturgbd/proposed"
        #     os.makedirs(save_dir, exist_ok=True)
        #     ts = datetime.now().strftime("%Y%m%d_%H%M%S")
